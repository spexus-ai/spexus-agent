...
`Open` takes exclusive coordinator filesystem lock, bootstraps schema v1 and immutable configuration. Close releases lock. cfg contains tenant/project, agents with SHA256 credentials only, exact profile snapshot bytes and static features. `RegisterFeature` is idempotent, immutable anchor/owner/scope; no API endpoint. `Ingest` is trusted-only and validates fixture channel/thread/actor, deduplicates source event, returns receipt and duplicate bool. Stop/Continue validate actor against feature allowlist. Continue refuses unresolved nonterminal work; no replay. `History` is local operator-only safe export (no credentials/profile prompt).
...
Errors: `{ "error": { "code": "...", "message": "...", "retryable": false }, "request_id": "UUID" }`, no internal stack/secret. Size128KiB, strict fields. APIError has Status and Code. UUIDs use NewID(). Payloads use json.Marshal exact exported types. CausationID nil serializes null. Profile snapshot is exact JSON bytes of TextProfile including prompt, tools:[],extensions:[]; Profile.Revision=Digest(snapshot). Model e.g. `openai-codex/gpt-6-luna`; reasoning `minimal`; exact configured allowlist. Runner must reject any tools or extensions before invoking Pi.
