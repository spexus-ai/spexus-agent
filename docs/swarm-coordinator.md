...
For an isolated preview fixture, start with `--local-fixture` instead of `--slack-config`. This mode opens no Slack Socket Mode connection and sends no Slack messages. With the coordinator stopped, inject one durable synthetic input using `inject-fixture --config /config/coordinator.json --state /state/coordinator.db --local-fixture --feature-id FEATURE_UUID --fixture-text '...'`, then restart `serve --local-fixture`. The fixture uses the configured feature's channel, thread, and first allowed actor. It does not authorize a production Slack rollout.
