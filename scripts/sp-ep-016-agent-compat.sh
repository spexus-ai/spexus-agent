...
#
# The agent authenticates only to Slack and invokes Pi as a local subprocess.
...
# tests use their existing fake Slack/Pi transports and never contact a shared
...
unexpected_http_files="$(rg -l -e 'net/http|http\.NewRequest|http\.Client' cmd internal --glob '*.go' --glob '!**/*_test.go' | rg -v '^(internal/slack/|internal/testsupport/)' || true)"
...
GOCACHE="${compat_go_cache}" go test ./internal/slack ./internal/piadapter ./internal/runtime ./internal/cli -count=1
git diff --check
