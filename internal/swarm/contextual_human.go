...
func (s *Store) BindContextualHumanAnswer(ctx context.Context, in SlackSource, kind string, selector int, answerText string) (string, error) {
	if s.cfg.WireVersion != 2 || in.SourceKind != "" || !uuid(in.FeatureID) || in.WorkspaceID != s.cfg.Human.WorkspaceID || !validSlackTS(in.MessageTS) || !validSlackTS(in.ThreadTS) || (kind != "answer" && kind != "deny") || selector < 0 || selector > 1000000 || !safeText(answerText, 16*1024) {
	...
	err := s.transaction(ctx, func(tx *sql.Tx) error {
		...
		digest := Digest([]byte(kind + "\x00" + strconv.Itoa(selector) + "\x00" + answerText + "\x00" + in.Text))
		...
		if selector != 0 && selected.RequestID == "" {
			return wireError(409, "unknown_human_selector")
		}
