...
func (r *Runner) input(ctx context.Context, d swarm.Delivery) (string, error) {
	if r.cfg.Role == "worker" {
		...
		if r.cfg.wireVersion() == 2 {
			prompt += ` If blocked by a decision you cannot make, finish this turn with outcome "blocked", nonempty summary, evidence array, error:null, and blocker:{"reason":"...","context":"...","question":"...","options":[{"id":"option-id","label":"..."}],"recommendation":"...","kind":"clarification|choice|permission|external_action|blocker"}. The blocker has exactly those six keys: do not add dependency_id or blocked_work. The coordinator creates the dependency ID, and the owner supplies blocked_work when asking the human. Use options:[] for a free-text question; recommend "нужна рекомендация человека" if you cannot recommend. Do not add blocker for succeeded/failed. Blocked is terminal for this attempt; do not wait, poll or invent human approval. On resume, treat the supplied decision as context for this new attempt only; do not claim broader rights or tools.`
		}
		return prompt + "\n" + string(b), e
	}
...
All action data is validated before any action is published. Do not claim acceptance or execution until a coordinator receipt exists. When requesting a human decision, set reply to the empty string because the question is posted separately. If the human asks for details without deciding, answer concisely with actions:[] and leave the question open.
`
	}
	if summaryOnly {
		prompt += `
